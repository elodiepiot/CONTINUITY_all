...
KMWDir="/NIRAL/work/maria5/EBDS_CIVILITY/LongitudinalConnectome_PostRegError/SubCortical_Processing/SALT/processing/RunFullDataset_202004/TxtLabels_Destrieux"

subcorticals=(Brainstem AmyL AmyR CaudL CaudR GPL GPR HippoL HippoR PutL PutR ThalL ThalR)
...
#BrainstemDest="${saltDIR}/${ID}-T1_SkullStripped_scaled_label_Brainstem_pp_surfSPHARM.vtk"

if [ -d "$OutFolder" ]; then 
	echo $OutFolder Found : Deleting 
        cmd="rm $OutFolder -r"
        eval $cmd
        cmd="mkdir $OutFolder"
        eval $cmd
...
##########
# Cleaning! Delete the old brainstem files in the SALT folder if any (run with old post processing) 
oldPPFile="${saltDIR}/${ID}-*Brainstem_pp.nrrd"
#echo $oldPPFile

if [ -f $oldPPFile ]; then
    echo Deleting $oldPPFile
    cmd="rm ${saltDIR}/${ID}-*Brainstem_pp_*" 
    eval $cmd
    cmd="rm ${oldPPFile}"
    eval $cmd
...
SPHARMSurf="${saltDIR}/${ID}-*_label_${region}_ppFinal_surfSPHARM.vtk"
#echo $SPHARMSurf

else 
SPHARMSurf="${saltDIR}/${ID}-*_label_${region}_pp*_surfSPHARM.vtk"
echo $SPHARMSurf
fi 
...
copysubsALL="/work/maria5/EBDS_CIVILITY/LongitudinalConnectome_PostRegError/SubCortical_Processing/Collected_SubcorticalsDWISpace/stx_${ID}_Destrieux_Labeled_Subcorticals_Combined_DWISpace.vtk"
...
if [ -f $inputMerge ]; then
	...
	copyMerge="/work/maria5/EBDS_CIVILITY/LongitudinalConnectome_PostRegError/SubCortical_Processing/Collected_FinalSurfaces/stx_${ID}_T1_CombinedSurface_white_Destrieux_WithSubcorticals.vtk"
